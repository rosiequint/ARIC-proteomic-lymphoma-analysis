...
Test
```{r}
test_model <- fit_tdc_models("SeqId_2665_26", "mm")
```
...
```{r}
allmodels_sp_summary <- results_singleprotein_alloutcomes %>%
  filter(term %in% c("PB", "PR", "PV", "PL")) %>%
  select(protein, outcome, model, term, estimate, p.value, std.error) %>%
  mutate(
...
```{r}
write_csv(allmodels_sp_summary, "tdc_allmodels_sp_summary.csv")
```